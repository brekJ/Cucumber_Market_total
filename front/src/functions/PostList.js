...
export const MyDealPostList = (listItems, loginUser) => {
  const MyDealPost = [];
  for (var i = 0; i < listItems.length; i++) {
    if (listItems[i].userID == loginUser.userID) MyDealPost.push(listItems[i]);
  }
  return MyDealPost;
};
...
export const MyDealDonePostList = (listItems, loginUser) => {
  ...
  for (var i = 0; i < listItems.length; i++) {
    if (
      listItems[i].user.userID == loginUser.userID &&
      listItems[i].postDealDone == true
    ) {
...
export const MyDealFalsePostList = (listItems, loginUser) => {
  ...
  for (var i = 0; i < listItems.length; i++) {
    if (
      listItems[i].user.userID == loginUser.userID &&
      listItems[i].postDealDone == false
    )
      MyDealPost.push(listItems[i]);
  }
  return MyDealPost;
